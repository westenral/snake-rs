...
impl GameState {
	...
	fn update(&mut self, dt: f64) {
		...
		if self.interval_time < UPDATE_INTERVAL { return; }
		self.interval_time -= UPDATE_INTERVAL;

		self.spos[0] += self.sdir[0];
		self.spos[1] += self.sdir[1];
		// branchless wrapping
		self.spos[0] += PLAYFIELD_WIDTH * ((self.spos[0] < 0.0) as u8 as f64 + -((self.spos[0] > PLAYFIELD_WIDTH - 1.0) as u8 as f64));
		self.spos[1] += PLAYFIELD_HEIGHT * ((self.spos[1] < 0.0) as u8 as f64 + -((self.spos[1] > PLAYFIELD_HEIGHT - 1.0) as u8 as f64));
	}

	fn button_event(&mut self, args: ButtonArgs) {
		if args.state == ButtonState::Release { return; }
		match args.button {
	...
	fn render(&self, args: RenderArgs, gl: &mut GlGraphics) {
		...
		gl.draw(args.viewport(), |c, g| {
			clear(BG_COLOR, g);
			rectangle(SNAKE_COLOR, [self.spos[0] * CELL_SIZE, self.spos[1] * CELL_SIZE, CELL_SIZE, CELL_SIZE], c.transform, g);
		})
	}
}
